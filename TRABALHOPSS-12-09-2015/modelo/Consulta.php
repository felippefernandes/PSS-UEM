<?php
	...
	class Consulta  {
		...
		public function buscaConsulta($codigo){
			$resultado = mysql_query("SELECT * FROM consulta C 
									  inner join pessoas P on P.id = C.id_paciente 
									  where C.id_consulta='".$codigo."'
									  ");
										
			$linha = mysql_fetch_array($resultado);
					$consulta = new Consulta();
					$consulta->setId($linha['id_consulta']);
					$consulta->setMedico($linha['id_medico']);
					$consulta->setPaciente($linha['id_paciente']);
					$consulta->setDataHora($linha['data_consulta']);
					$consulta->setSintoma($linha['sintomas']);
					$consulta->setDiagnostico($linha['diagnostico']);
					$consulta->setReceita($linha['receita']);
			return $consulta;
		}
		...
		public function alterar(){
			...
			mysql_query("UPDATE consulta SET 
						sintomas='".$this->getSintoma()."', 
						diagnostico='".$this->getDiagnostico()."', 
						receita='".$this->getReceita()."' 
						where id_consulta = ".$this->getId()."    ");
